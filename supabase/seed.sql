...
-- Clean existing data (in reverse dependency order, ignore if tables are empty)
DELETE FROM fiches WHERE true;
DELETE FROM chapters WHERE true;
DELETE FROM subjects WHERE true;
DELETE FROM series WHERE true;
...
BEGIN
  ...
  INSERT INTO chapters (id, subject_id, slug, number, title, description, display_order) VALUES
    ...
    (v_ch_croissance_eco_id, v_economie_id, 'croissance-economique-developpement', 2, 'Croissance économique et développement', 'Définition, IDH, développement durable', 2),
    ...
    (v_ch_ped_id, v_economie_id, 'les-pays-en-developpement', 6, 'Les pays en développement', 'Stratégies agricoles et industrielles, financement du développement', 6),
